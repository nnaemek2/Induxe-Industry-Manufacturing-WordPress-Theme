...
$post_social_share_on =induxe_get_opt( 'post_social_share_on', false );
$post_subtitle = induxe_get_page_opt('post_subtitle');
$post_tags_on =induxe_get_opt( 'post_tags_on', false );
$post_navigation_on =induxe_get_opt( 'post_navigation_on', true );
$post_author_box_on =induxe_get_opt( 'post_author_box_on', false );
$post_feature_image_on =induxe_get_opt( 'post_feature_image_on', false );
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('single-hentry'); ?>>
    <?php induxe_post_meta(); ?>
    <?php if(!empty($post_subtitle)): ?>
        <h3 class="sub-post-title">
            <?php echo esc_attr( $post_subtitle ); ?>
        </h3>
    <?php endif; ?>
    ...
    <div class="entry-body">
        ...
        <?php if($post_tags_on || $post_social_share_on) : ?>
            <div class="entry-content-bottom">
                <?php if($post_tags_on) :
                   induxe_entry_tagged_in();
                endif;
                if($post_social_share_on) :
                   induxe_socials_share_default(); 
                endif; ?>
            </div>
        <?php endif; ?>
        ...
        <?php if($post_navigation_on) : ?>
            <div class="entry-navigation">
                <?php induxe_post_nav_default(); ?>
            </div>
        <?php endif; ?>

        <?php induxe_related_post(); ?>
    </div>

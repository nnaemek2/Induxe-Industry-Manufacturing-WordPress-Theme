...
$team_content_list2 = (array) vc_param_group_parse_atts( $content_list2 );
wp_enqueue_script( 'waypoints' );
wp_enqueue_script( 'vc_waypoints' );
wp_enqueue_style( 'vc_animate-css' );
?>

<div id="<?php echo esc_attr($html_id) ?>" class="ct-grid ct-grid-team3">

    <div class="ct-grid-inner ct-grid-masonry row" data-gutter="15">
        <div class="grid-sizer <?php echo esc_attr($grid_sizer); ?>"></div>
        <?php foreach ($team_content_list2 as $key => $value) {
                        ...
                        <div class="team-social pxl-empty">
                            <ul class="inner-social">
                                <?php foreach ($el_social as $key => $value) {
                                    ...
                                    <?php if ( !empty($social_link) && !empty($icon_class) ) { ?>
                                        <li><a href="<?php echo esc_url($social_link); ?>" target="_blank"><i class="<?php echo esc_attr( $icon_class ); ?>"></i></a></li>
                                    <?php } ?>
                                <?php } ?>
                            </ul>
                        </div>

...
class Recent_Posts_Widget extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            'ct_recent_posts',
            esc_html__( '* Recent Posts', 'induxe' ),
            array(
                'description' => esc_html__( 'Shows your most recent posts.', 'induxe' ),
    ...
    function widget( $args, $instance )
    {
        $instance = wp_parse_args( (array) $instance, array(
            'title'         => esc_html__( 'Recent Posts', 'induxe' ),
            'number'        => 4,
            'post_in'        => '',
            'show_featured_image' => '',
        ) );
        $title = empty( $instance['title'] ) ? esc_html__( 'Recent Posts', 'induxe' ) : $instance['title'];
            ...
            while ( $r->have_posts() )
            {
                ...
                echo '<div class="entry-brief">';
                    if($show_featured_image == 'show') :
                        ...
                        $thumbnail_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'induxe-thumbnail', false); ?>
    ...
    function form( $instance )
    {
        ...
        $title         = $instance['title'] ? esc_attr( $instance['title'] ) : esc_html__( 'Recent Posts', 'induxe' );
        $number        = absint( $instance['number'] );
...
add_action( 'widgets_init', 'induxe_register_recent_widget' );
function induxe_register_recent_widget(){

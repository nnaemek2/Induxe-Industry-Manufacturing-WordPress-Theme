...
/**
 * Recent Service widgets
 *
 * @package Induxe
 * @version 1.0
 */

class Recent_service_Widget extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            'ct_recent_service',
            esc_html__( '* Recent Service', 'induxe' ),
            array(
                'description' => esc_html__( 'Shows your most recent service.', 'induxe' ),
    ...
    function widget( $args, $instance )
    {
        $instance = wp_parse_args( (array) $instance, array(
            'title'         => esc_html__( 'Recent Service', 'induxe' ),
            'number'        => 4,
            'post_in'        => '',
            'show_featured_image'        => '',
        ) );

        $title = empty( $instance['title'] ) ? esc_html__( 'Recent Service', 'induxe' ) : $instance['title'];
    ...
    function form( $instance )
    {
        ...
        $title         = $instance['title'] ? esc_attr( $instance['title'] ) : esc_html__( 'Recent Service', 'induxe' );
        $number        = absint( $instance['number'] );
        ...
        <p><label for="<?php echo esc_url($this->get_field_id('post_in')); ?>"><?php esc_html_e( 'Post in', 'induxe' ); ?></label>
         ...
         <p><label for="<?php echo esc_url($this->get_field_id('show_featured_image')); ?>"><?php esc_html_e( 'Featured Image', 'induxe' ); ?></label>
         <select class="widefat" id="<?php echo esc_attr( $this->get_field_id('show_featured_image') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_featured_image') ); ?>">
            <option value="show"<?php if( $show_featured_image == 'show' ){ echo 'selected="selected"';} ?>><?php esc_html_e('Show', 'induxe'); ?></option>
            <option value="hide"<?php if( $show_featured_image == 'hide' ){ echo 'selected="selected"';} ?>><?php esc_html_e('Hide', 'induxe'); ?></option>
         </select>
         </p>
...
add_action( 'widgets_init', 'induxe_register_recent_sv_widget' );
function induxe_register_recent_sv_widget(){
    if(function_exists('ct_allow_RegisterWidget')){
        ct_allow_RegisterWidget( 'Recent_service_Widget' );
    }
}